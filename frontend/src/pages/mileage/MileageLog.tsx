...
import { Table, Button, Modal, Form, Input, InputNumber, DatePicker, Select, Space, Popconfirm, Tag, message as antdMessage } from 'antd';
...
const MileageLog: FC = () => {
  ...
  const columns: TableColumnsType<MileageLog> = [
    ...
    },
    {
      ...
      render: (_, r) => (
        <Space size="small">
          ...
          {r.status === 'draft' && (
            <Button size="small" type="primary" icon={<SendOutlined />} onClick={() => handleSubmit(r.id)}>
              {t('submit')}
            </Button>
          )}
