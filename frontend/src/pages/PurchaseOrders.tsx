...
const PurchaseOrders: React.FC = () => {
  ...
  const fetchData = async () => {
    setLoading(true);
    try { const r = await api.get('/api/purchase-orders', { params: { page, page_size: 20 } }); setData(r.data.items); setTotal(r.data.total); }
  ...
  return (
    <div>
      <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 16, alignItems: 'center', gap: 12, flexWrap: 'wrap' }}>
        ...
        <Button type="primary" icon={<PlusOutlined />} onClick={openNew}>{t('new_purchase_order')}</Button>
      </div>
      ...
      <Modal open={modalOpen} onCancel={() => setModalOpen(false)} title={t('new_purchase_order')} width={900} footer={null} destroyOnHidden>
