...
import { Card, Row, Col, Statistic, Tabs, DatePicker, Button, Space, Tag, message, Image } from 'antd';
...
export default function EInvoiceDashboard() {
 ...
 <Space>
 <Button icon={<QrcodeOutlined />} onClick={() => showQr(r.invoice_id)} />
 <Button icon={<SendOutlined />} disabled={['accepted', 'submitted'].includes(r.status || '')}
 onClick={() => submit(r.invoice_id)} />
 <Button icon={<RetweetOutlined />} disabled={!['rejected', 'failed'].includes(r.status || '')}
 ...
 key: 'actions',
 render: (_: unknown, r: Submission) => (
 <Button icon={<RetweetOutlined />} onClick={() => retry(r.invoice_id)}>{t('retry')}</Button>
 ),
 },
 ];

 return (
 <div style={{ padding: 16 }} data-section-id="einvoice.dashboard">
 <Space style={{ marginBottom: 16 }} wrap>
 <h2 style={{ margin: 0 }}>{t('einvoice_dashboard')}</h2>
 <HelpIcon sectionId="einvoice.dashboard" />
 <RangePicker
 value={range}
 ...
 {showSkeleton && !report ? <LoadingSkeleton variant="card" /> : (
 <>
 ...
 children: (
 <ResponsiveTableAdapter rowKey="id" loading={loading} dataSource={report?.items || []} columns={columns} pagination={{ pageSize: 20 }} />
 ),
 },
 {
 key: 'errors',
 ...
 children: (
 <ResponsiveTableAdapter rowKey="id" loading={loading} dataSource={errors} columns={errorColumns} pagination={{ pageSize: 20 }} />
 ),
 },
 ]}
 />
 ...
 title={t('qr_code')}
 open={!!qrInvoiceId}
 onClose={() => setQrInvoiceId(null)} hideFooter
 >
 {qrImage ? (
