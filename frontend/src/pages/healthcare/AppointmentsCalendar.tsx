...
import { Calendar, Badge, Card, Button, Drawer, Form, Input, Select, DatePicker, TimePicker, message, Space, Tag, Popconfirm } from 'antd';
      ...
      <Card title={`${t('healthcare.appointments_on')} ${selectedDate.format('YYYY-MM-DD')}`} style={{ marginTop: 16 }}>
        <Space direction="vertical" style={{ width: '100%' }}>
          ...
          {selectedDayAppointments.map((apt) => {
            ...
            return (
              <Card key={apt.id} size="small" style={{ marginBottom: 8 }}>
                <Space direction="vertical" style={{ width: '100%' }}>
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                    ...
                    <Space>
                      <Button size="small" onClick={() => handleEdit(apt)}>{t('edit')}</Button>
                      {apt.status === 'scheduled' && (
                        <Button size="small" type="primary" onClick={() => void handleStatusChange(apt.id, 'in_progress')}>
                          {t('healthcare.check_in')}
                        </Button>
                      )}
                      {apt.status === 'in_progress' && (
                        <Button size="small" onClick={() => void handleStatusChange(apt.id, 'completed')}>
