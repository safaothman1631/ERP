...
import { Table, Button, Tag, Modal, Form, Input, Select, DatePicker, Space, Popconfirm } from 'antd';
...
const SerialNumbers: React.FC = () => {
  ...
  const columns = [
    ...
    { title: t('expiry_date'), dataIndex: 'expiry_date', key: 'expiry_date', render: (d?: string) => d?.substring(0, 10) },
    {
      ...
      render: (_: unknown, r: Serial) => (
        <Space>
          <Button size="small" icon={<EditOutlined />} onClick={() => openEdit(r)}>{t('edit')}</Button>
        </Space>
      ),
    },
  ];
  ...
  return (
    <div>
      ...
      <Table rowKey="id" columns={visibleColumns} dataSource={data} loading={loading} pagination={{ pageSize: 50 }} />

      <Modal
        open={modalOpen}
        onCancel={() => setModalOpen(false)}
        title={editingId ? t('edit_serial') : t('new_serial')}
        footer={null}
        destroyOnHidden
      >
