...
import { Button, Form, Input, Select, DatePicker, Space, Modal, Radio } from 'antd';
...
export default function SalesReturns() {
 ...
 try { return JSON.parse(localStorage.getItem('salesReturns.hiddenCols') || '[]'); } catch { return []; }
 });
 ...
 const res = await api.get('/api/returns/sales', { params });
 ...
 if (editId) {
 await api.put(`/api/returns/sales/${editId}`, payload);
 message.success(t('updated'));
 } else {
 await api.post('/api/returns/sales', payload);
 ...
 onOk: async () => {
 try {
 await api.delete(`/api/returns/sales/${id}`);
 ...
 // Kit list tabs (All / Draft / Open / Closed) — server-side filtered via status param.
 const tabs: KitListTab[] = [
 { key: 'all', label: t('all', 'All') },
 { key: 'draft', label: t('draft', 'Draft') },
 { key: 'open', label: t('open', 'Open') },
 { key: 'closed', label: t('closed', 'Closed') },
 ];

 const allColumns = [
 {
 title: t('return_number'),
 ...
 return num
 ? <span style={{ color: 'var(--ink-700)', fontFamily: 'var(--font-mono)', fontSize: 12.5 }}>{num}</span>
 ...
 <div>
 <PageHeader
 title={t('sales_returns')}
 subtitle={t('sales_returns_subtitle', 'Sales returns')}
 sectionId="sales.returns"
 extra={
 <Space>
 <Button
 type="primary"
 ...
 onTabChange={(k) => {
 setFilterStatus(k === 'all' ? undefined : k);
 ...
 <Radio value="closed">{t('closed', 'Closed')}</Radio>
 </Radio.Group>
 </KitFiltersButton>
 <KitStatusFilter
 ...
 downloadCsv('sales-returns', filteredData, cols);
 }}
