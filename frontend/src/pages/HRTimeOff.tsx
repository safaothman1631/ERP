...
import { Card, Table, Button, Modal, Form, Select, DatePicker, Input, Space, Tag, message, Tabs } from 'antd';
...
export default function HRTimeOff() {
  ...
  const cols = [
      ...
      render: (s: string) => <Tag color={s === 'approved' ? 'green' : s === 'rejected' ? 'red' : 'orange'}>{s}</Tag> },
    {
      ...
      render: (_: unknown, r: TimeOff) => r.status === 'pending' ? (
        <Space>
          <Button size="small" type="primary" icon={<CheckOutlined />} onClick={() => approve(r.id)}>{t('approve')}</Button>
          <Button size="small" danger icon={<CloseOutlined />} onClick={() => reject(r.id)}>{t('reject')}</Button>
        </Space>
      ) : null,
    },
  ];
  ...
  return (
    <div style={{ padding: 16 }}>
        ...
        },
        {
          ...
          children: (
            <>
              ...
              <Card><Table rowKey="id" dataSource={types} columns={typeCols} pagination={false} /></Card>
            </>
          ),
        },
      ]} />

      <Modal open={open} onOk={submit} onCancel={() => setOpen(false)} title={t('new_request')} destroyOnHidden>
      ...
      <Modal open={typeOpen} onOk={submitType} onCancel={() => setTypeOpen(false)} title={t('new_leave_type')} destroyOnHidden>
