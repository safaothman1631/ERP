...
import {
  Table, Button, Space, Tabs, Tag, Modal, Form, Input, Select, message, Drawer,
  Steps, Avatar, Tooltip, DatePicker, Checkbox
} from 'antd';
import {
          ...
          {record.status === 'pending' && (
            <>
              <Tooltip title={t('dms.send_reminder')}>
                <Button icon={<BellOutlined />} size="small" onClick={() => handleRemind(record.id)} />
              </Tooltip>
              <Tooltip title={t('dms.cancel')}>
                <Button icon={<CloseCircleOutlined />} size="small" danger onClick={() => handleCancel(record.id)} />
              </Tooltip>
            </>
          )}
        </Space>
          ...
          items={[
            {
              key: 'sent',
              label: t('dms.sent'),
              children: <Table dataSource={sentRequests} columns={sentColumns} loading={loading} rowKey="id" pagination={{ pageSize: 20 }} />,
            },
            {
              key: 'received',
              label: t('dms.received'),
              children: <Table dataSource={receivedRequests} columns={receivedColumns} loading={loading} rowKey="id" pagination={{ pageSize: 20 }} />,
            },
            {
              key: 'templates',
              label: t('dms.templates'),
              children: <div style={{ padding: space.lg, textAlign: 'center' }}>{t('dms.templates_coming_soon')}</div>,
            },
          ]}
        />
      </div>
          ...
          <Space direction="vertical" size="large" style={{ width: '100%' }}>
            ...
            {selectedRequest.status === 'pending' && (
              <>
                ...
                <Button type="primary" size="large" block icon={<CheckCircleOutlined />} onClick={handleSign}>
                  {t('dms.sign')}
                </Button>
              </>
            )}
