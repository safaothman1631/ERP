...
import {
  Table, Button, Space, Upload, Input, Modal, Form, Select, Tag, Tree, Card,
  Tooltip, Row, Col, DatePicker, Checkbox, message, Drawer, List, Avatar
} from 'antd';
import {
      ...
      render: (_: any, record: any) => (
        <Space>
          <Tooltip title={t('dms.preview')}>
            <Button icon={<FileOutlined />} size="small" onClick={() => navigate(`/dms/${record.id}`)} />
          </Tooltip>
          ...
          <Tooltip title={t('dms.share')}>
            <Button icon={<ShareAltOutlined />} size="small" onClick={() => handleShare(record)} />
          </Tooltip>
          <Tooltip title={t('dms.versions')}>
            <Button icon={<HistoryOutlined />} size="small" onClick={() => handleVersions(record)} />
          </Tooltip>
  ...
  return (
    <div>
      ...
      <Row gutter={16} style={{ marginTop: space.md }}>
        <Col span={5}>
          <Card title={t('dms.folders')} size="small">
            <Tree
              treeData={treeData}
              defaultExpandAll
