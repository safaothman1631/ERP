...
import {
  Card, Descriptions, Button, Space, Table, Tabs, Input, message, Modal,
  Form, Select, Tag, List, Avatar, Upload, Tooltip, DatePicker
} from 'antd';
import {
...
const DocumentDetail: React.FC = () => {
      ...
      render: (_: any, record: any) => (
        <Space>
          <Tooltip title={t('dms.download')}>
            <Button icon={<DownloadOutlined />} size="small" onClick={() => window.open(record.storage_url, '_blank')} />
          </Tooltip>
          <Tooltip title={t('dms.restore')}>
            <Button icon={<RollbackOutlined />} size="small" onClick={() => handleRestoreVersion(record.id)} />
          </Tooltip>
        </Space>
      ),
    },
  ...
  const shareColumns = [
    ...
    { title: t('dms.expires_at'), dataIndex: 'expires_at', key: 'expires_at', render: (v: string) => v ? new Date(v).toLocaleDateString() : '—' },
    {
      ...
      width: 100,
      render: (_: any, record: any) => (
        <Button size="small" danger onClick={() => handleRevokeShare(record.id)}>
          {t('dms.revoke')}
        </Button>
      ),
    },
  ...
  return (
    <div>
          ...
          <Space direction="vertical" size="large" style={{ width: '100%' }}>
            ...
            <div style={{ textAlign: 'center', background: '#fafafa', padding: space.lg, borderRadius: 8 }}>
              {doc.mime_type.includes('pdf') ? (
                ...
                <img src={doc.storage_url} alt={doc.name} style={{ maxWidth: '100%', maxHeight: 600 }} />
              ) : (
            ...
            items={[
              {
                ...
                children: (
                  <>
                    ...
                    <Table dataSource={versions} columns={versionColumns} rowKey="id" pagination={{ pageSize: 10 }} />
                  </>
                ),
              },
              {
              ...
              },
              {
                ...
                children: (
                  <>
                    ...
                    <Table dataSource={shares} columns={shareColumns} rowKey="id" pagination={{ pageSize: 10 }} />
                  </>
                ),
              },
            ]}
      ...
      <Modal
        ...
        onCancel={() => { setVersionModalOpen(false); versionForm.resetFields(); }}
