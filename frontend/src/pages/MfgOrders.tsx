...
import { Card, Button, Form, Select, InputNumber, DatePicker, Input, Space, Tag, message, Descriptions } from 'antd';
...
export default function MfgOrders() {
 ...
 <Space>
 <Button onClick={() => showMO(r.id)}>{t('view')}</Button>
 {r.status === 'draft' && <Button type="primary" icon={<CheckOutlined />} onClick={() => confirmMO(r.id)} />}
 {r.status === 'confirmed' && <Button type="primary" onClick={() => doneMO(r.id)}>{t('done')}</Button>}
 {r.status !== 'done' && (
 <Button danger icon={<DeleteOutlined />} onClick={() => removeMO(r.id)} />
 )}
 </Space>
 ),
 },
 ...
 return (
 <div style={{ padding: 16 }} data-section-id="manufacturing.orders">
 <Space style={{ marginBottom: 12 }}>
 <h2 style={{ margin: 0 }}>{t('manufacturing_orders')}</h2>
 <HelpIcon sectionId="manufacturing.orders" />
 ...
 <FormDialog open={open} onOk={submit} onClose={() => setOpen(false)} title={t('new_mo')}>
 ...
 <FormDialog open={!!drawer} onClose={() => setDrawer(null)} title={drawer?.number}>
 {drawer && (
 <>
 ...
 <Space>
 {r.status === 'pending' && <Button icon={<PlayCircleOutlined />} onClick={() => startWO(r.id)}>{t('start')}</Button>}
 {r.status === 'in_progress' && <Button type="primary" icon={<CheckOutlined />} onClick={() => finishWO(r.id)}>{t('finish')}</Button>}
 </Space>
 ),
 },
 ]}
