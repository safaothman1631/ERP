...
import { Table, Button, Space, Modal, Form, Input, Select, Switch, Tag, message, Card, Drawer } from 'antd';
  ...
  const columns = [
    ...
    },
    {
      ...
      render: (_: any, record: any) => (
        <Space>
          <Button icon={<EditOutlined />} size="small" onClick={() => navigate(`/automation/workflows/${record.id}`)} />
          <Button icon={<CopyOutlined />} size="small" onClick={() => handleDuplicate(record)} />
          <Button icon={<PlayCircleOutlined />} size="small" onClick={() => navigate(`/automation/workflows/${record.id}/runs`)} />
  ...
  return (
    <div>
      ...
      <Card style={{ marginTop: space.md }}>
        <Table dataSource={workflows} columns={columns} loading={loading} rowKey="id" />
      </Card>
