...
import { Table, Button, Space, Input, Drawer, Form, Select, InputNumber, Tag, Row, Col, Card, Statistic } from 'antd';
...
const ConstructionProjects: React.FC = () => {
  ...
  const columns: TableColumnsType<Project> = [
    ...
    },
    {
      ...
      render: (_: any, rec: Project) => (
        <Button type="link" size="small" icon={<EyeOutlined />} onClick={() => handleViewDetail(rec)}>
  ...
  return (
    <>
      ...
      <Drawer title={t('construction.add_project')} open={drawer} onClose={() => setDrawer(false)} width={500}>
      ...
      >
        {selectedProject && (
          <Space direction="vertical" size="large" style={{ width: '100%' }}>
            <Card title={t('construction.project_info')} size="small">
              <Row gutter={[16, 16]}>
                <Col span={12}>
            ...
            <Card title={t('construction.cost_summary')} size="small" loading={loadingCost}>
              {costSummary && (
                <Row gutter={[16, 16]}>
                  <Col span={12}>
                    <KpiCard
