...
import { Table, Button, Space, Tag, Modal, Form, Input, InputNumber, App, Drawer, DatePicker } from 'antd';
  ...
  const columns = [
    ...
    },
    {
      ...
      render: (_: any, record: GiftCard) => (
        <Space>
          {!record.is_active && (
            <Button size="small" type="primary" onClick={() => handleActivate(record)}>
              {t('activate')}
            </Button>
          )}
