...
import {
  Card, Tabs, Form, Input, Switch, Button, Table, Tag, Space, Modal, Row, Col, Statistic,
...
export default function WhatsApp() {
  ...
  const tplCols = [
    ...
    { title: t('locale'), dataIndex: 'locale' },
    {
      ...
      render: (_: unknown, r: Template) => (
        <Space>
          <Button size="small" onClick={() => { setTplModal(r); tplForm.setFieldsValue(r); }}>{t('edit')}</Button>
  ...
  return (
    <div>
      <PageHeader
        title={t('whatsapp')}
        subtitle={t('whatsapp_subtitle', 'پەیامی واتسئەپ بۆ کڕیاران')}
        extra={
          <Space size={spaceTk.sm}>
            <Button icon={<ReloadOutlined />} onClick={load}>{t('refresh')}</Button>
            <Button type="primary" icon={<SendOutlined />} size="large" onClick={() => setSendModal(true)}>
              {t('send_message')}
            </Button>
          </Space>
        }
          ...
          },
          {
            ...
            children: (
              <>
                ...
                <Table rowKey="id" loading={loading} dataSource={templates} columns={tplCols} />
              </>
            ),
          },
          {
            key: 'messages',
            label: t('message_log'),
            children: <Table rowKey="id" loading={loading} dataSource={messages} columns={msgCols} pagination={{ pageSize: 25 }} />,
          },
        ]}
      />
