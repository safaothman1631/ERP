...
import { Table, Button, Tag, Modal, Form, Input, Select, Space, Popconfirm, InputNumber } from 'antd';
      ...
      <Table
        ...
        expandable={{
          expandedRowRender: (record: PriceList) => (
            <Table dataSource={record.items || []} columns={nestedColumns} rowKey="item_id" pagination={false} size="small" />
          ),
        }}
      />

      <Modal open={modalOpen} onCancel={() => setModalOpen(false)} title={editingId ? t('edit') : t('priceLists')} footer={null} destroyOnHidden width={700}>
            ...
            <tbody>
              {priceItems.map(p => (
                <tr key={p.key}>
                  ...
                  <td style={{ padding: 4 }}>
                    <Button icon={<DeleteOutlined />} size="small" danger onClick={() => setPriceItems(prev => prev.filter(x => x.key !== p.key))} />
                  </td>
                </tr>
              ))}
            </tbody>
