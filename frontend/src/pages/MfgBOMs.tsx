...
export default function MfgBOMs() {
  ...
  return (
    <div style={{ padding: 16 }}>
      ...
      <Modal open={open} onOk={save} onCancel={() => setOpen(false)} title={editing ? t('edit_bom') : t('new_bom')} destroyOnHidden width={720}>
              ...
              <>
                ...
                {fields.map(({ key, name }) => (
                  <Space key={key} style={{ display: 'flex' }} align="baseline">
                    ...
                    <Button danger size="small" onClick={() => remove(name)} icon={<DeleteOutlined />} />
                  </Space>
                ))}
