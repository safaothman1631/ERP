...
import { Tabs, Table, Button, Tag, Modal, Form, Input, InputNumber, Select, Space, DatePicker, Popconfirm, Row, Col, Card, Statistic, Progress, Empty, Typography } from 'antd';
...
const Assets: React.FC = () => {
  ...
  const columns = [
    ...
    { title: t('purchasePrice'), dataIndex: 'purchase_price', key: 'purchase_price', render: (v: number) => <Text strong>{fmtIQD(v)}</Text> },
    {
      ...
      render: (v: number, r: Asset) => {
        ...
        return (
          <Space orientation="vertical" size={2} style={{ minWidth: 120 }}>
            <Text strong>{fmtIQD(v)}</Text>
            <Progress percent={pct} size="small" strokeColor={pct >= 100 ? '#f59e0b' : '#1677ff'} showInfo={false} className="depreciation-progress" />
    ...
    },
    {
      ...
      render: (_: unknown, r: Asset) => (
        <Space>
          ...
          {r.status === 'active' && (
            <Button size="small" onClick={() => { disposeForm.resetFields(); setDisposeModal(r.id); }}>
              {t('dispose')}
            </Button>
          )}
  ...
  return (
    <div>
      <PageHeader
        title={t('assets', 'سامانە کۆن‌ـەکان')}
        subtitle={t('assets_subtitle', 'بەڕێوەبردنی سامان و کرتە بەهاکان')}
        extra={
