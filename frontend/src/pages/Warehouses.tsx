...
import { Tabs, Table, Button, Tag, Modal, Form, Input, Switch, Space, Popconfirm, Select, InputNumber, DatePicker, Empty, Typography } from 'antd';
      ...
      <Table
        ...
        expandable={{
          expandedRowRender: (record: Warehouse) => (
            <Table dataSource={record.stock_items || []} columns={stockColumns} rowKey="item_id" pagination={false} size="small" />
          ),
        }}
      />

      <Modal open={modalOpen} onCancel={() => setModalOpen(false)} title={editingId ? t('edit') : t('warehouses')} footer={null} destroyOnHidden>
...
const StockTransfers: React.FC = () => {
  ...
  return (
    <div>
      ...
      <Modal open={modalOpen} onCancel={() => setModalOpen(false)} title={t('stockTransfer')} footer={null} destroyOnHidden width={700}>
            ...
            <tbody>
              {transferItems.map(ti => (
                <tr key={ti.key}>
                  ...
                  <td style={{ padding: 4 }}>
                    <Button icon={<DeleteOutlined />} size="small" danger onClick={() => setTransferItems(prev => prev.filter(x => x.key !== ti.key))} />
                  </td>
                </tr>
              ))}
            </tbody>
