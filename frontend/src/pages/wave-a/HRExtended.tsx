...
import { Table, Button, Space, Modal, Form, Input, Select, Tag, message, Card, Tabs } from 'antd';
...
const HRExtended: React.FC = () => {
  ...
  const applicationsColumns = [
    ...
    { title: t('hr_extended.status'), dataIndex: 'status', key: 'status', render: (v: string) => <Tag color={statusColors[v] || 'default'}>{t(`hr_extended.status_${v}`)}</Tag> },
    {
      ...
      render: (_: any, record: any) => (
        <Space>
          <Button icon={<DeleteOutlined />} size="small" danger onClick={() => handleDelete(record.id, 'applications')} />
        </Space>
      ),
    },
  ];
  ...
  const appraisalsColumns = [
    ...
    { title: t('hr_extended.status'), dataIndex: 'status', key: 'status', render: (v: string) => <Tag color={v === 'completed' ? 'green' : 'orange'}>{t(`hr_extended.appraisal_status_${v}`)}</Tag> },
    {
      ...
      render: (_: any, record: any) => (
        <Space>
          <Button icon={<DeleteOutlined />} size="small" danger onClick={() => handleDelete(record.id, 'appraisals')} />
        </Space>
      ),
    },
  ];
      ...
      <Card style={{ marginTop: space.md }}>
        <Tabs activeKey={activeTab} onChange={setActiveTab}>
          <Tabs.TabPane tab={t('hr_extended.recruitment')} key="recruitment">
            <Table dataSource={applications} columns={applicationsColumns} loading={loading} rowKey="id" />
          </Tabs.TabPane>
          <Tabs.TabPane tab={t('hr_extended.appraisals')} key="appraisals">
            <Table dataSource={appraisals} columns={appraisalsColumns} loading={loading} rowKey="id" />
          </Tabs.TabPane>
        </Tabs>
      </Card>
