  ...
  const columns = [
    ...
    { title: t('field_service.worker'), dataIndex: 'assigned_worker_id', key: 'assigned_worker_id', render: (v: string) => v || t('field_service.unassigned') },
    {
      ...
      render: (_: any, record: any) => (
        <Space>
          <Button icon={<UserAddOutlined />} size="small" onClick={() => openAssign(record)} title={t('field_service.assign_worker')} />
          {record.status !== 'done' && (
            <Button icon={<CheckCircleOutlined />} size="small" type="primary" onClick={() => handleComplete(record.id)} />
          )}
  ...
  return (
    <div>
      ...
      <Modal
        ...
        onCancel={() => { setAssignModalOpen(false); assignForm.resetFields(); setSelectedOrder(null); }}
