    ...
    { title: t('quality.measure'), dataIndex: 'measure', key: 'measure' },
    {
      ...
      render: (_: any, record: any) => (
        <Space>
          {!record.result && (
            <>
              <Button icon={<CheckCircleOutlined />} size="small" type="primary" onClick={() => handlePass(record.id)} title={t('quality.pass')} />
              <Button icon={<CloseCircleOutlined />} size="small" danger onClick={() => handleFail(record.id)} title={t('quality.fail')} />
            </>
          )}
  ...
  return (
    <div>
      ...
      <Card style={{ marginTop: space.md }}>
        <Table dataSource={checks} columns={columns} loading={loading} rowKey="id" />
      </Card>
