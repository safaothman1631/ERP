...
import { Table, Button, Space, Input, Modal, Form, InputNumber} from 'antd';
...
const Items: React.FC = () => {
  ...
  const allColumns = [
    ...
    { title: t('stock'), dataIndex: 'stock_on_hand', key: 'stock_on_hand' },
    {
      ...
      render: (_: any, record: any) => (
        <Space>
          <Button icon={<EditOutlined />} size="small" onClick={() => { setEditing(record); form.setFieldsValue(record); setModal(true); }} />
  ...
  return (
    <div>
      <PageHeader
        title={t('items')}
        subtitle={t('items_subtitle', 'کاڵا و خزمەتگوزارییەکان')}
        helpKey="items"
        extra={
          <Space size={space.sm}>
            <ExportButton endpoint="/api/export/products" filename="products" />
            <Button type="primary" icon={<PlusOutlined />} size="large" onClick={() => navigate('/items/new')}>{t('new_item')}</Button>
          </Space>
        }
      />
      {backendUnavailable ? (
      ...
      <Modal title={editing ? t('edit') : t('new_item')} open={modal} onCancel={() => { setModal(false); setEditing(null); form.resetFields(); }} onOk={() => form.submit()} width={600}>
