  ...
  const columns = [
    ...
    { title: t('status'), dataIndex: 'status', key: 'status', render: (s: string) => <Tag color={statusColors[s]}>{t(s)}</Tag> },
    {
      ...
      render: (_: any, r: any) => {
        ...
        return menuitems.length ? <Dropdown menu={{ items: menuitems }} trigger={['click']}><Button icon={<MoreOutlined />} size="small" /></Dropdown> : null;
      },
    },
  ];
  ...
  return (
    <div>
      <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 16, alignItems: 'center', gap: 12, flexWrap: 'wrap' }}>
        ...
        <Button type="primary" icon={<PlusOutlined />} onClick={openNew}>{t('new_vendor_credit')}</Button>
      </div>
      ...
      <Modal open={modalOpen} onCancel={() => setModalOpen(false)} title={t('new_vendor_credit')} width={900} footer={null} destroyOnHidden>
