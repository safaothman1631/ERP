...
import GlassSaveButton from '../../components/glass/GlassSaveButton';
...
const sectionSubtitle = (key: SectionKey, t: (k: string, fb?: string) => unknown): string => {
  ...
  switch (key) {
    ...
    case 'integrations_health': return tt('int_health_desc', 'Live readiness of e-invoice, payments, messaging, and outbound integrations.');
...
const SecuritySettings: React.FC = () => {
  ...
  const [mustKeep, setMustKeep] = useState<boolean>(false);
  ...
  const loadMe = useCallback(async () => {
    try {
      ...
      const isEnabled = Boolean(r.data?.is_2fa_enabled);
      const requiresSetup = Boolean(r.data?.requires_2fa_setup);
      setEnabled(isEnabled);
      setMandatory(requiresSetup);
      setMustKeep(Boolean(r.data?.must_keep_2fa));
      return { isEnabled, requiresSetup };
    } catch {
      return { isEnabled: false, requiresSetup: false };
    }
  }, []);
  ...
  useEffect(() => {
    void loadMe().then(({ isEnabled, requiresSetup }) => {
      if (requiresSetup && !isEnabled && !autoSetupStarted.current) {
  ...
  const verifySetup = async () => {
    ...
    try {
      ...
      setMustKeep(true);
      setSetupOpen(false);
      window.dispatchEvent(new CustomEvent('2fa-setup-complete'));
      ...
      {mandatory && !enabled && (
        <Alert
          ...
          description={t(
            ...
            'Your role requires 2FA. Complete setup below to continue using the app.',
          )}
        />
      )}
      <SectionCard
...
const ModulesSettings: React.FC = () => {
  ...
  const pendingAdminCount = useOnboardingStore(s => s.pendingAdminCount);
      ...
      >
        ...
        {pool && pool.length > 0 && (
          <div style={{ paddingTop: space.lg }}>
            <div style={{ fontSize: fontSize.sm, fontWeight: 600, color: palette.ink500, marginBottom: space.md }}>
              {t('license_pool_readonly', 'Available modules (set by vendor)')}{' '}
              {license.bundleId && <Tag>{license.bundleId}</Tag>}
            </div>
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(min(220px, 100%), 1fr))', gap: space.md }}>
              {pool.map(m => (
                <ModuleSettingsCard
                  key={m.key}
                  module={m}
                  enabled={!!enabled?.some(e => e.key === m.key)}
                  inPool
                />
              ))}
            </div>
          ...
          <div style={{ paddingTop: space.lg }}>
            <Link to="/settings/module-requests">
              <Button icon={<TeamOutlined />}>
                {t('modreq_page_title', 'Module requests')}
        ...
        {enabled && enabled.length > 0 && (
          <div style={{ paddingTop: space.lg }}>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(min(220px, 100%), 1fr))', gap: space.md }}>
              {enabled.map(m => (
                <ModuleSettingsCard key={m.key} module={m} enabled inPool />
              ))}
            </div>
          </div>
        )}
...
const SaveBar: React.FC<{ dirty: boolean; saving: boolean; onSave: () => void; onReset?: () => void }> = ({ dirty, saving, onSave, onReset }) => {
  const { t } = useTranslation();
  const canEdit = useSettingsEdit();
  if (!dirty || !canEdit) return null;
  const glass = getGlassStyle('dialog', true);
  return (
    <div
      style={{
        position: 'sticky',
    ...
    >
      ...
      <GlassSaveButton loading={saving} onClick={onSave} />
    </div>
  );
};
...
const SmsWhatsappSettings: React.FC = () => {
  ...
  return (
    <Space direction="vertical" size="large" style={{ width: '100%' }}>
      ...
      >
        <SettingsRow label={t('sm_provider', 'Provider')}>
          <Select
            value={values.provider}
            onChange={(v) => setValue('provider', v as SmsWaBag['provider'])}
            disabled={loading}
            style={{ width: '100%' }}
            options={[
              { value: 'local', label: t('local_provider', 'Local Provider') },
              { value: 'twilio', label: 'Twilio' },
              { value: 'vonage', label: 'Vonage' },
              { value: 'meta_wa', label: 'Meta WhatsApp' },
            ]}
          />
        </SettingsRow>
        <SettingsRow label={t('sm_sender', 'Sender ID')}><Input value={values.sender_id} onChange={(e) => setValue('sender_id', e.target.value)} disabled={loading} style={{ width: '100%' }} /></SettingsRow>
        <SettingsRow label={t('sm_sid', 'Account SID / API key')}><Input value={values.account_sid} onChange={(e) => setValue('account_sid', e.target.value)} disabled={loading} style={{ width: '100%' }} /></SettingsRow>
        <SettingsRow label={t('sm_auth', 'Auth token')}><Input.Password value={values.auth_token} onChange={(e) => setValue('auth_token', e.target.value)} disabled={loading} style={{ width: '100%' }} /></SettingsRow>
        <SettingsRow label={t('sm_wa_api', 'WhatsApp Phone Number ID')}><Input value={values.wa_phone_number_id} onChange={(e) => setValue('wa_phone_number_id', e.target.value)} disabled={loading} style={{ width: '100%' }} /></SettingsRow>
        <SettingsRow label={t('sm_otp', 'OTP message template')}><Input.TextArea value={values.otp_template} onChange={(e) => setValue('otp_template', e.target.value)} disabled={loading} rows={2} style={{ width: '100%' }} /></SettingsRow>
        <SettingsRow label={t('sm_quota', 'Daily quota')}><InputNumber min={0} max={1000000} step={100} value={values.daily_quota} onChange={(v) => setValue('daily_quota', Number(v) || 0)} disabled={loading} style={{ width: '100%' }} /></SettingsRow>
      </SectionCard>
      <SaveBar dirty={dirty} saving={saving} onSave={save} />
    </Space>
  );
