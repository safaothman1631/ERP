...
import type { PrintTemplateProps, InvoiceDocument } from './types';

/**
 * InvoicePrintTemplate — A4 print template for Sales Invoices.
 ...
 * - @media print hides .sidebar, .topbar, .no-print (Requirement 19.5, 19.7)
 */
export const InvoicePrintTemplate: React.FC<PrintTemplateProps> = ({
  document,
  company,
  isRTL,
  currency,
}) => {
  const doc = document as InvoiceDocument;
  ...
  return (
    <BasePrintTemplate
      ...
      from={company}
      toLabel={labels.to}
      toName={doc.customerName}
      toAddress={doc.customerAddress}
      toPhone={doc.customerPhone}
      toEmail={doc.customerEmail}
      toTaxNumber={doc.customerTaxNumber}
