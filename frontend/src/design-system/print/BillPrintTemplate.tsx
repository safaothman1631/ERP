...
import type { PrintTemplateProps, BillDocument } from './types';

/**
 * BillPrintTemplate — A4 print template for Vendor Bills (Accounts Payable).
 ...
 */
export const BillPrintTemplate: React.FC<PrintTemplateProps> = ({
  document,
  company,
  isRTL,
  currency,
}) => {
  const doc = document as BillDocument;
  ...
  // For a bill, the "from" party is the vendor and "to" is the company
  // We display the vendor info in the "to" slot and company in the "from" slot
  const vendorInfo = {
    name: doc.vendorName,
    address: doc.vendorAddress,
    phone: doc.vendorPhone,
    email: doc.vendorEmail,
    taxNumber: doc.vendorTaxNumber,
  };
  ...
  return (
    <BasePrintTemplate
      ...
      from={company}
      toLabel={labels.to}
      toName={vendorInfo.name}
      toAddress={vendorInfo.address}
      toPhone={vendorInfo.phone}
      toEmail={vendorInfo.email}
      toTaxNumber={vendorInfo.taxNumber}
