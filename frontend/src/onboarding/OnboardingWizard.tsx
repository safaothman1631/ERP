...
import {
  Modal, Steps, Card, Row, Col, Button, Checkbox, Tag, Empty, Space,
  Typography, Divider, Input, Alert, theme as antdTheme, Grid, App as AntApp,
} from 'antd';
import {
...
export default function OnboardingWizard({ open, firstTime = false, onClose, onComplete }: OnboardingWizardProps) {
  ...
  return (
    <Modal
      open={open}
      onCancel={firstTime ? undefined : onClose}
      closable={!firstTime}
      footer={null}
      width={920}
      style={{ maxWidth: '95vw', top: isMobile ? 16 : 60 }}
      destroyOnHidden
      mask={{ closable: false }}
      keyboard={!firstTime}
      styles={{ body: { padding: isMobile ? '14px 14px' : '20px 24px' } }}
      title={
        <Space>
          <RocketOutlined style={{ color: accent }} />
          <span style={{ fontSize: 16, fontWeight: 600 }}>{t('onb_welcome')}</span>
        </Space>
      }
    >
      {firstTime && (
