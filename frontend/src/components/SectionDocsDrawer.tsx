...
import { Drawer, Tag, Divider, Typography, Space } from 'antd';
import {
  InfoCircleOutlined,
  TeamOutlined,
  DatabaseOutlined,
    ...
    >
      {doc && (
        <>
          ...
          {/* Who uses */}
          {sectionHeader(<TeamOutlined />, t('docs.who_uses', 'کێ بەکاری دێنێت'))}
          <Space wrap size={[6, 6]}>
            {doc.whoUses.map((r) => (
          ...
          {/* Sub-areas */}
          {sectionHeader(<AppOutlined />, t('docs.sub_areas', 'بەش و ناوچەکان'))}
          ...
          {doc.mermaidDiagram && (
            <>
              {sectionHeader(<ArrowRightOutlined />, t('docs.data_flow', 'ڕێگای داتا'))}
              <MermaidDiagram src={doc.mermaidDiagram} />
            </>
          )}

          {/* Data destination */}
          {sectionHeader(<DatabaseOutlined />, t('docs.data_store', 'کۆگای داتا'))}
          ...
          {doc.related.length > 0 && (
            <>
              {sectionHeader(<LinkOutlined />, t('docs.related', 'پەیوەندیدار'))}
              <Space wrap size={[6, 6]}>
                {doc.related.map((k) => (
                  <Tag
                    key={k}
          ...
          {doc.kpis && doc.kpis.length > 0 && (
            <>
              {sectionHeader(<BulbOutlined />, t('docs.kpis', 'KPI گرنگەکان'))}
          ...
          {doc.tips && doc.tips.length > 0 && (
            <>
              {sectionHeader(<BulbOutlined />, t('docs.tips', 'تێبینی گرنگ'))}
              <ul style={{ paddingInlineStart: 18, margin: 0 }}>
                {doc.tips.map((tip) => (
