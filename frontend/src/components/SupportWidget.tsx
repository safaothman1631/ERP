...
import { Button, Drawer, Tag, Typography, Space, Divider, Alert } from 'antd';
import {
        ...
        >
          ...
          <div style={{ flex: 1 }}>
            ...
            <Space size={6} style={{ marginTop: 6 }}>
              <Button size="small" type="primary" onClick={() => { setOpen(true); setNudge(false); }}>
                {t('open_help')}
              </Button>
              <Button size="small" type="text" onClick={dismissNudge} icon={<CloseOutlined />} />
            </Space>
          </div>
        </div>
      )}
